...
async function sendExpoPush(
  ...
  for (const chunk of chunks) {
    ...
    try {
      ...
      logger.info(
        `Expo push sent: ${chunk.length - failures.length} ok, ${failures.length} failed`
      );
    } catch (err) {
...
async function sendPushToUsers(
  ...
  const expoTokens = users.map((u) => u.expoPushToken).filter((t): t is string => !!t);
  await Promise.all([
...
export const onBloodRequestCreated = onDocumentCreated(
  ...
  async (event) => {
    ...
    if (!snap) return;
    const request = snap.data() as BloodRequestDoc;
    ...
    const body =
      `${request.bloodType} blood needed at ${request.hospitalName} — ` +
...
export const broadcastAnnouncement = onCall(async (request) => {
  ...
  let query: FirebaseFirestore.Query = db.collection(USERS).where("role", "==", "user");
  if (bloodType) {
...
export const deleteUserAccount = onCall(async (request) => {
  ...
  if (uid === request.auth?.uid) {
    throw new HttpsError("failed-precondition", "Admins cannot delete their own account this way.");
  }
